...
export async function main(ns) {
	...
	var target = await SelectServer(ns);
...
/** @param {NS} ns **/
async function SelectServer(ns) {
	for (var id = servers_to_hack.length - 1; id > -1; id--) {
		var target = servers_to_hack[id];

		if (ns.getHackingLevel() >= ns.getServerRequiredHackingLevel(servers_to_hack[id])) {
			return target;
		}
	}
}
